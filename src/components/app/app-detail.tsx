...
import { createApp, getAppLink } from '@/lib/poketto'
...
export const AppDetail = ({ app, comments }: { app: AppWithRelation, comments: IAppComment[] }) => {
	const { addApp, apps, delApp } = useAppStore()
	const router = useRouter()
	const hasGot = Boolean(apps.find((c) => c.poketto.id === app.id))
	...
	const onDelApp = () => {
		delApp(app.id)
		const nextApp = apps.find((c) => c.poketto.id !== app.id)!
		void router.push(getAppLink(nextApp.poketto.id))
		scrollTo({ y: 0 })
	}
	
	return (<section id={'poketto-app-detail'}
