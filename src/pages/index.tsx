...
export default function WorkspacesPage() {
	...
	const { data: users } = api.user.getAllUser.useQuery()
	const user = useUser()
	console.log('[workspaces] ', { user, users })
		...
		<div className={'w-full h-full'}>
			<Card id={'recent'} variant={'ghost'} className={'flex flex-col gap-4'}>
				...
				<CardContent className={'w-full | flex flex-col divide-y'}>
					<WorkspaceItem wid={user?.id ?? 'own'} title={'My Workspace'} icon={<PersonIcon/>}/>
					<WorkspaceItem wid={'team'} title={'Team Workspace'} icon={<UsersIcon className={ICON_DIMENSION_SM}/>}/>
				</CardContent>
			</Card>
...
const WorkspaceItem = ({ wid, title, avatar, icon }: {
	...
	return (<Link href={`/w/${wid}`} className={'w-full p-2'}>
