...
export async function POST(req: Request) {
  ...
  if (baseEnv.KV_REST_API_URL && baseEnv.KV_REST_API_TOKEN) {
    ...
    if (!success) {
      return NextResponse.json(
        { message: "Hey man, you are too fast! Please slow down!" },
        {
          status: 429,
          headers: {
  ...
  const context = await proxy.message.getContext.query({ conversationId, content, modelType })
  ...
  const apiKey = baseEnv.OPENAI_API_KEY
  ...
  // if (response. !== 200) {
  //   return NextResponse.json("服务器发送错误，请稍后再试！", { status: 500 })
  // }
