...
export const flowgpt2poketto_conversation = (f: IFlowgptConversation): IPokettoConversation => ({
	createdAt: d(f.createdAt).toDate(), messages: f.messages.map((m) => ({
		...m, type: 'user', format: 'text', appId: f.id, uid: undefined, parentId: undefined, interactions: {}, id: nanoid(), createdAt: new Date(), role: m.role as 'system' | 'user' | 'assistant' | 'function',
	})),
})
export const flowgpt2pokettoApp = (p: IFlowgptPromptBasic | FlowgptPromptFull): AppWithRelation => {
	...
	return {
		...
		id: p.id, name: p.title, avatar: p.thumbnailURL, categoryId: p.categoryId, language: p.language, desc: p.description, updatedAt: dayjs(p.updatedAt).toDate(), createdAt: dayjs(p.createdAt).toDate(), platform: PlatformType.FlowGPT, version: DEFAULT_APP_VERSION, creatorId: p.userId, creator: {
